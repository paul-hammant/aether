...
void bench_single_core_throughput() {
    ...
    // Wait for processing
    while (atomic_load(&actor->count) < MSGS && (get_time_ms() - start) < 5000) {
        sleep_ms(1);
    }
    
    long end = get_time_ms();
    int processed = atomic_load(&actor->count);
...
void bench_multi_core_throughput(int cores) {
    ...
    while (1) {
        ...
        for (int i = 0; i < cores; i++) {
            total_processed += atomic_load(&actors[i]->count);
        }
...
void bench_cross_core_overhead() {
    ...
    while (atomic_load(&actor1->count) < MSGS && (get_time_ms() - start) < 5000) {
        sleep_ms(1);
    }
    
    long end = get_time_ms();
    int processed = atomic_load(&actor1->count);
    double elapsed = (end - start) / 1000.0;
...
void bench_latency() {
    ...
    for (int i = 0; i < SAMPLES; i++) {
        int before = atomic_load(&actor->count);
        long start = get_time_ms();
        ...
        while (atomic_load(&actor->count) <= before && get_time_ms() < timeout) {
            // Tight loop for accuracy
        }
        
        long end = get_time_ms();
        if (atomic_load(&actor->count) > before) {
...
void bench_burst_patterns() {
    ...
    while (atomic_load(&actor->count) < total_sent && (get_time_ms() - start) < 10000) {
        sleep_ms(10);
    }
...
void bench_mailbox_saturation() {
    ...
    while (atomic_load(&actor->count) < MSGS && (get_time_ms() - start) < 10000) {
        sleep_ms(10);
    }
